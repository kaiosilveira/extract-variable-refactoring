...
Often times, expressions become hard to read and adds a lot of overhead when trying to understand a given piece of code. Extracting a variable for a complex expression helps the reader by reducing the amount of logic to interpret, replacing it basically by well structured text.

## Working examples

For this refactoring we are going to analyze two working examples that calculates the final price of an order: a functional approach and an Object-Oriented approach.
...
- Customers should have a 5% of discount per item for every item, starting after 500 units
...
And then we can replace the shipping calculation by its variable:
...
For this example, we have an `Order` class that holds the data for a given `record` and knows how to calculate its final price. The code inside the `price` method is actually the same used in the functional example. To refactor that, we:
